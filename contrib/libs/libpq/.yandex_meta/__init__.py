...
def post_install(self):
    m = self.yamakes["."]
    # Take some functions from libc_compat, not from port/*.c
    for port in SRCS_FROM_LIBC_COMPAT:
        m.SRCS.remove(port)
        os.remove(os.path.join(self.dstdir, port))
...
libpq = NixProject(
    owners=["floatdrop", "g:cpp-contrib"],
