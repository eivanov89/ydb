    ...
    int orderID, int districtID, int warehouseID, int carrierID)
{
    ...
    static std::string query = std::format(R"(
        ...
        INSERT INTO `oorder` (O_ID, O_D_ID, O_W_ID, O_CARRIER_ID)
...
TTransactionTask GetDeliveryTask(TTransactionContext& context,
    NThreading::TPromise<TStatus> promise,
    TSession session)
{
    auto& Log = context.Log;
    LOG_D("Terminal " << context.TerminalID << " started Delivery transaction");
    ...
    for (int districtID = DISTRICT_LOW_ID; districtID <= DISTRICT_HIGH_ID; districtID++) {
        ...
        if (!newOrderResult.IsSuccess()) {
            ...
            promise.SetValue(newOrderResult);
        ...
        if (!orderParser.TryNextRow()) {
            LOG_D("Terminal " << context.TerminalID << " no new orders for district " << districtID);
            continue;
        }
        ...
        if (!customerIdResult.IsSuccess()) {
            ...
            promise.SetValue(customerIdResult);
            co_return TTransactionResult(TTransactionResult::E_ERROR);
        }
        ...
        int customerID = customerParser.ColumnParser("O_C_ID").GetInt32();
        ...
        if (!customerDataResult.IsSuccess()) {
            ...
            promise.SetValue(customerDataResult);
            co_return TTransactionResult(TTransactionResult::E_ERROR);
        }
        ...
        double customerBalance = customerDataParser.ColumnParser("C_BALANCE").GetDouble();
        double deliveryCount = customerDataParser.ColumnParser("C_DELIVERY_CNT").GetInt32();
        ...
        while (orderLinesParser.TryNextRow()) {
            ...
            double amount = orderLinesParser.ColumnParser("OL_AMOUNT").GetDouble();
        ...
        if (!deleteOrderResult.IsSuccess()) {
            ...
            promise.SetValue(deleteOrderResult);
            co_return TTransactionResult(TTransactionResult::E_ERROR);
        }
        ...
        if (!updateCarrierResult.IsSuccess()) {
            ...
            promise.SetValue(updateCarrierResult);
            co_return TTransactionResult(TTransactionResult::E_ERROR);
        }
        ...
        for (int lineNumber : orderData.OrderLineNumbers) {
            ...
            if (!updateDeliveryDateResult.IsSuccess()) {
                ...
                promise.SetValue(updateDeliveryDateResult);
                co_return TTransactionResult(TTransactionResult::E_ERROR);
            }
        }
       ...
       auto updateCustomerFuture = UpdateCustomerBalanceAndDeliveryCount(
        ...
        if (!updateCustomerResult.IsSuccess()) {
            ...
            promise.SetValue(updateCustomerResult);
            co_return TTransactionResult(TTransactionResult::E_ERROR);
        }
        ...
        LOG_D("Terminal " << context.TerminalID
            << " processed delivery for district " << districtID
            << ", order " << orderID
            << ", customer " << customerID
    ...
    LOG_D("Terminal " << context.TerminalID
        << " finished Delivery transaction successfully, processed " << processedOrderCount << " districts");

    promise.SetValue(TStatus(EStatus::SUCCESS, NIssue::TIssues()));
