...
TTransactionTask GetStockLevelTask(TTransactionContext& context,
    NThreading::TPromise<TStatus> promise,
    TSession session)
{
    auto& Log = context.Log;
    LOG_D("Terminal " << context.TerminalID << " started StockLevel transaction");
    ...
    int nextOrderID = districtParser.ColumnParser("D_NEXT_O_ID").GetInt32();
    ...
    if (!stockCountResult.IsSuccess()) {
        ...
        promise.SetValue(stockCountResult);
        co_return TTransactionResult(TTransactionResult::E_ERROR);
    }

    LOG_D("Terminal " << context.TerminalID << " finished StockLevel transaction successfully");
