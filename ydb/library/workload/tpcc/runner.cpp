...
void TPCCRunner::UpdateDisplayTextMode(const TCalculatedStatusData& data) {
    ...
    if (data.ProgressPercent > 0) {
        ss << std::fixed << std::setprecision(1) << " (" << data.ProgressPercent << "%, "
           ...
           << (int)((data.RemainingMinutes * 60) % 60) << " remaining)";
    }

    ss << " | Efficiency: " << std::setprecision(1) << data.Efficiency << "% | "
       << "tpmC: " << std::setprecision(1) << LastStatisticsSnapshot->Tpmc << " | "
       << "Running: " << data.RunningTerminals << " terminals, " << data.RunningTransactions << " transactions";
    ...
    for (size_t i = 0; i < GetEnumItemsCount<ETransactionType>(); ++i) {
        ...
        std::cout << std::left
                  << std::setw(15) << ToString(static_cast<ETransactionType>(i))
...
void TPCCRunner::UpdateDisplayTuiMode(const TCalculatedStatusData& data) {
    ...
    // Header section with efficiency, tpmC, and progress
    ...
    metricsSs << std::fixed << std::setprecision(1)
              ...
              << "Running: " << data.RunningTerminals << " terminals, "
              << data.RunningTransactions << " transactions";

    std::stringstream timingSs;
    timingSs << data.Phase << " - " << data.ElapsedMinutes << ":"
             << std::setfill('0') << std::setw(2) << data.ElapsedSeconds << " elapsed";
    if (data.ProgressPercent > 0) {
        timingSs << "   ETA: " << data.RemainingMinutes << " min";
    }

    // Calculate progress ratio for gauge
    float progressRatio = static_cast<float>(data.ProgressPercent / 100.0);

    auto headerSection = vbox({
        text(headerSs.str()),
        text(metricsSs.str()),
        text(timingSs.str()),
        hbox({
            text("Progress: "),
            gauge(progressRatio) | size(WIDTH, EQUAL, 30),
    ...
    txRows.push_back(hbox({
        ...
        separator(),
        text("OK/s") | size(WIDTH, EQUAL, 8),
        separator(),
        text("Fail/s") | size(WIDTH, EQUAL, 8),
        separator(),
        text("p50ms") | size(WIDTH, EQUAL, 8),
        separator(),
        text("p90ms") | size(WIDTH, EQUAL, 8),
        separator(),
        text("p99ms") | size(WIDTH, EQUAL, 8)
    }));

    for (size_t i = 0; i < GetEnumItemsCount<ETransactionType>(); ++i) {
        const auto& stats = LastStatisticsSnapshot->GlobalStats[i];

        std::stringstream okSs, failSs, p50Ss, p90Ss, p99Ss;
        okSs << std::fixed << std::setprecision(1) << stats.OkPerSecond;
        ...
        p90Ss << std::fixed << std::setprecision(0) << stats.LatencyHistogramFullMs.GetValueAtPercentile(90);
        p99Ss << std::fixed << std::setprecision(0) << stats.LatencyHistogramFullMs.GetValueAtPercentile(99);

        txRows.push_back(hbox({
            text(ToString(static_cast<ETransactionType>(i))) | size(WIDTH, EQUAL, 12),
            separator(),
            text(okSs.str()) | size(WIDTH, EQUAL, 8),
            separator(),
            text(failSs.str()) | size(WIDTH, EQUAL, 8),
            separator(),
            text(p50Ss.str()) | size(WIDTH, EQUAL, 8),
            separator(),
            text(p90Ss.str()) | size(WIDTH, EQUAL, 8),
            separator(),
            text(p99Ss.str()) | size(WIDTH, EQUAL, 8)
        }));
    }

    auto txTable = vbox(txRows);
    ...
    auto logsPanel = vbox(logElements) | vscroll_indicator | flex;

    // Middle row: Transaction stats (left) and logs (right)
    auto middleRow = hbox({
        txTable | size(WIDTH, LESS_THAN, 60),
        separator(),
        logsPanel | flex
    }) | size(HEIGHT, LESS_THAN, 12);

    // Per-thread statistics section (bottom)
    Elements threadElements;

    // Create header row for thread stats
    threadElements.push_back(hbox({
        text("Thr") | size(WIDTH, EQUAL, 4),
        separator(),
        text("Load") | size(WIDTH, EQUAL, 12),
        separator(),
        text("Terminals/s") | size(WIDTH, EQUAL, 12),
        separator(),
        text("QPS") | size(WIDTH, EQUAL, 12),
        separator(),
        text("Inflight") | size(WIDTH, EQUAL, 8),
        separator(),
        text("Queue p50") | size(WIDTH, EQUAL, 10),
        separator(),
        text("Queue p90") | size(WIDTH, EQUAL, 10)
    }));

    // Add thread statistics rows
    for (size_t i = 0; i < LastStatisticsSnapshot->StatVec.size(); ++i) {
        const auto& stats = LastStatisticsSnapshot->StatVec[i];
        double load = stats.ExecutingTime / stats.TotalTime;

        // Create load bar visualization
        auto loadBar = gauge(static_cast<float>(load)) | size(WIDTH, EQUAL, 8);

        std::stringstream loadSs, termSs, qpsSs, inflightSs, p50Ss, p90Ss;
        loadSs << std::fixed << std::setprecision(2) << (load * 100) << "%";
        termSs << std::fixed << std::setprecision(1) << stats.TerminalsPerSecond;
        qpsSs << std::fixed << std::setprecision(1) << stats.QueriesPerSecond;
        inflightSs << stats.TaskThreadStats->InternalTasksWaitingInflight;
        p50Ss << std::fixed << std::setprecision(1) << stats.InternalInflightWaitTimeMs.GetValueAtPercentile(50);
        p90Ss << std::fixed << std::setprecision(1) << stats.InternalInflightWaitTimeMs.GetValueAtPercentile(90);

        threadElements.push_back(hbox({
            text(std::to_string(i)) | size(WIDTH, EQUAL, 4),
            separator(),
            hbox({loadBar, text(" " + loadSs.str())}) | size(WIDTH, EQUAL, 12),
            separator(),
            text(termSs.str()) | size(WIDTH, EQUAL, 12),
            separator(),
            text(qpsSs.str()) | size(WIDTH, EQUAL, 12),
            separator(),
            text(inflightSs.str()) | size(WIDTH, EQUAL, 8),
            separator(),
            text(p50Ss.str()) | size(WIDTH, EQUAL, 10),
            separator(),
            text(p90Ss.str()) | size(WIDTH, EQUAL, 10)
        }));
    }

    auto threadSection = vbox(threadElements);
...
void TPCCRunner::DumpFinalStats() {
    ...
    for (size_t i = 0; i < GetEnumItemsCount<ETransactionType>(); ++i) {
        ...
        std::cout << std::setw(15) << ToString(static_cast<ETransactionType>(i))
