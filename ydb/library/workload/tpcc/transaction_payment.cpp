    ...
    int districtID, int warehouseID, double amount, const TString& data)
{
    ...
    auto params = TParamsBuilder()
        ...
        .AddParam("$h_c_nano_ts").Int64(time(nullptr)).Build()
        .Build();
...
TTransactionTask GetPaymentTask(TTransactionContext& context,
    NThreading::TPromise<TStatus> promise,
    TSession session)
{
    auto& Log = context.Log;
    LOG_D("Terminal " << context.TerminalID << " started Payment transaction");
    ...
    if (!warehouseResult.IsSuccess()) {
        ...
        promise.SetValue(warehouseResult);
    ...
    std::string warehouseName = warehouseParser.ColumnParser("W_NAME").GetUtf8();
    ...
    if (!districtResult.IsSuccess()) {
        ...
        promise.SetValue(districtResult);
        co_return TTransactionResult(TTransactionResult::E_ERROR);
    }
    ...
    std::string districtName = districtParser.ColumnParser("D_NAME").GetUtf8();
    ...
    if (customer.c_credit == "BC") {
        ...
        if (!updateCustResult.IsSuccess()) {
            ...
            promise.SetValue(updateCustResult);
            co_return TTransactionResult(TTransactionResult::E_ERROR);
        }
    } else {
    ...
    if (!historyResult.IsSuccess()) {
        ...
        promise.SetValue(historyResult);
        co_return TTransactionResult(TTransactionResult::E_ERROR);
    }

    LOG_D("Terminal " << context.TerminalID << " finished Payment transaction successfully: "
