...
void TCommandLatency::Config(TConfig& config) {
    ...
    config.Opts->AddLongOption(
        "chain-work-usec", TStringBuilder() << "Amount of work for each actor in the chain (ActorChain kind only)")
            .OptionalArgument("INT").StoreResult(&ChainConfig->WorkUsec).DefaultValue(ChainConfig->WorkUsec);
    config.Opts->AddLongOption(
        "no-tail-chain", TStringBuilder() << "Don't use Tail sends and registrations (ActorChain kind only)")
            .NoArgument().SetFlag(&ChainConfig->NoTailChain).DefaultValue(ChainConfig->NoTailChain);
}
