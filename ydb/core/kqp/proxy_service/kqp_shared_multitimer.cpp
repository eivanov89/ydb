...
public:
        ...
        std::unique_ptr<IEventHandle> ev) override
    {
        ...
        ScheduleDeadlineIfNeeded(ctx, deadline, delta);
        return id;
    }
...
private:
    void ScheduleDeadlineIfNeeded(const TActorContext &ctx, TMonotonic deadline, TDuration delta) {
        ...
        if (changed) {
            ...
            if (delta <= LONG_TIMER_THRESHOLD) {
                ctx.ExecutorThread.Schedule(deadline, new IEventHandle(TimerActor, TimerActor, new TEvents::TEvWakeup()));
            } else {
...
private:
    ...
    void Wakeup(const TActorContext &ctx) {
        ...
        if (!TimeoutQueue.empty()) {
            ...
            MultiTimer->ScheduleDeadlineIfNeeded(ctx, nextTimer->Deadline, nextTimer->Delta);
        }
    }
...
std::pair<TMultiTimerMtSafePtr, NActors::TActorId> CreateMultiTimer() {
    ...
    TActorId id = TlsActivationContext->ExecutorThread.RegisterActor(timerActor);
    multiTimer->Init(id);
