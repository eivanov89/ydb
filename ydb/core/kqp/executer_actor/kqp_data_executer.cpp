...
public:
    ...
    TKqpDataExecuter(
        ...
        : TBase(maybeTxId, target, std::move(request), database, userToken, counters, tableServiceConfig,
...
IActor* CreateKqpDataExecuter(
    const std::optional<ui64>& maybeTxId,
    const TActorId& target,
    IKqpGateway::TExecPhysicalRequest&& request, const TString& database, const TIntrusiveConstPtr<NACLib::TUserToken>& userToken,
...
{
    return new TKqpDataExecuter(
        maybeTxId,
        target,
        std::move(request), database, userToken, counters, streamResult, tableServiceConfig,
